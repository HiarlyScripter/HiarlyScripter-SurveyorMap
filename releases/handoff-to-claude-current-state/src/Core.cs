...
namespace SurveyorMap
{
    ...
    public sealed class SurveyorMapController : MonoBehaviour
    {
        ...
        private bool _enemyScanStatsDirty;
        private float _nextEnemySummaryLog;
        ...
        internal void Initialize(CapabilityReport report)
        {
            ...
            LogHelper.Info("Marker system bypassed for native HUD stabilization phase.");
            IsInitialized = true;
        }
        ...
        private void CreateHud()
        {
            ...
            GameObject contentObject = new GameObject("MapRoot");
        ...
        private void UpdateSelfMarkerOverlay()
        {
            if (_selfMarker == null)
                return;
        ...
        private int UpdateEnemyMarkers(Vector3 center, float markerScale, float now)
        {
            ...
            for (int i = 0; i < _enemies.Count; i++)
            {
                ...
                Vector2 point;
                if (!_nativeMapCapture.TryProjectWorldToHud(GameApi.GetEnemyPosition(enemy, center), _markerRoot, _config.ClampMarkersToBounds.Value, out point))
                {
                    if (_hasEnemyScanStats)
                ...
                marker.SetColor(EnemyColor(tier, now));
        ...
        private void TryLogEnemyScanSummary(float now)
        {
            ...
            bool changed = _lastEnemySummary != summary;
            if (_lastEnemySummary != null && now < _nextEnemySummaryLog)
                return;

            if (!changed && !_enemyScanStatsDirty)
                return;

            _lastEnemySummary = summary;
            _nextEnemySummaryLog = now + 5f;
            _enemyScanStatsDirty = false;
            LogHelper.Info(summary);
    ...
    internal static class GameApi
    {
        ...
        private static EnemyParent GetEnemyParent(Enemy enemy)
        {
