...
namespace SurveyorMap
{
    ...
    public sealed class SurveyorMapPlugin : BaseUnityPlugin
    {
        ...
        private void Update()
        {
            ...
            {
                float now = Time.unscaledTime;
                bool inLevel = IsInLevelDirect();

                // ── Proof-of-life rate-limited log (every 5 s, only when in level) ──────────
                // This log PROVES Plugin.Update is executing in the level.
                // If this never appears in LogOutput.log during a run, the plugin lifecycle
                // is broken at the MonoBehaviour level (not at the gate/capture level).
                if (inLevel && now >= _proofLogNext)
                {
                    _proofLogNext = now + 5f;
                    bool forceProof = Settings != null && Settings.ForceHudProofOfLife.Value;
                    LogHelper.Info("Plugin.Update alive in level. frame=" + Time.frameCount +
                                   ", forceProof=" + forceProof +
                                   ", proofHudVisible=" + _proofHudVisible +
                                   ", controller=" + (_controller != null) +
                                   ", hudCreated=" + (_controller != null && _controller.HudCreated) +
                                   ", canvasActive=" + (_controller != null && _controller.HudActive));
                }

                // ── Toggle M for proof-of-life HUD ─────────────────────────────────────────
                if (Settings != null && Input.GetKeyDown(Settings.ToggleKey.Value) &&
                    _toggleHandledFrame != Time.frameCount)
                {
                    _toggleHandledFrame = Time.frameCount;
                    if (Settings.ForceHudProofOfLife.Value)
                    {
                        ...
                        LogHelper.Info("Toggle key detected in plugin Update (proof): " + Settings.ToggleKey.Value +
                                       ", proofHudVisible=" + _proofHudVisible);
                    }
                    else
                    {
            ...
            {
                // Full stack trace so we can find silent exceptions killing the loop.
                LogHelper.RateLimitedError("plugin-update", "Plugin.Update exception:\n" + ex, 5f);
            }
        }

        // Equivalent to dig-Minimap StateUtils.IsInLevel() — direct field access, no reflection.
        ...
        private void OnGUI()
        {
            // ── Proof-of-Life overlay ───────────────────────────────────────────────────────
            // Completely bypasses Canvas, controller, RawImage, NativeMapCapture and all gates.
            // If this box appears in a level, Plugin.OnGUI is alive → bug is in gate/capture,
            // not in the lifecycle. If it never appears, the plugin itself is being killed.
            if (Settings != null && Settings.ForceHudProofOfLife.Value &&
                _proofHudVisible && IsInLevelDirect())
            {
                if (!_proofHudFirstLog)
                {
                    _proofHudFirstLog = true;
                    LogHelper.Info("HUD proof visible=True (ForceHudProofOfLife=True, IsInLevelDirect=True).");
                }

                float w = 260f;
                float h = 260f;
                float x = 24f;
                // BottomLeft 260x260 at y=120 from bottom → in GUI (y from top): Screen.height-120-260
                ...
                GUI.DrawTexture(new Rect(x + w - 3f, y, 3f, h), Texture2D.whiteTexture);

                // Label
                GUI.color = Color.white;
                GUI.Label(new Rect(x + 8f, y + 8f, w - 16f, 24f), "SurveyorMap LIVE");
                GUI.Label(new Rect(x + 8f, y + 28f, w - 16f, 20f),
                          "frame=" + Time.frameCount + "  M=toggle");

                GUI.color = prev;
            }

            // ── Optional debug overlay (only when EnableDebugOverlay=true) ────────────────
