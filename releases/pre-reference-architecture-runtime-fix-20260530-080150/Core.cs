    ...
    public sealed class SurveyorMapPlugin : BaseUnityPlugin
    {
        ...
        private void Update()
        {
            try
            {
                ...
                // Restart RuntimeLoop if heartbeat lost (coroutine died silently)
                ...
                // Fallback toggle key (handles M even if RuntimeLoop is dead; frame guard prevents double-fire)
                if (Settings != null && Input.GetKeyDown(Settings.ToggleKey.Value) &&
                    _toggleHandledFrame != Time.frameCount)
                {
                    _toggleHandledFrame = Time.frameCount;
                ...
                // Fallback: ensure controller alive and drive RuntimeUpdate
                // The _lastRuntimeUpdateFrame guard in RuntimeUpdateCore makes this a no-op
                // if RuntimeLoop or controller.Update already ran this frame.
            ...
            {
                LogHelper.RateLimitedError("plugin-update", "Plugin.Update exception: " + ex.Message, 5f);
            }
        }
