...
namespace SurveyorMap
{
    ...
    public sealed class SurveyorMapPlugin : BaseUnityPlugin
    {
        ...
        private void Awake()
        {
            ...
            try
            {
                ...
                System.DateTime ts = System.IO.File.GetLastWriteTime(dllPath);
                byte[] raw = System.IO.File.ReadAllBytes(dllPath);
    ...
    internal sealed class SurveyorMapConfig
    {
        ...
        {
            if (_layoutDefaultsVersion.Value >= 6)
                return;
            ...
            ShowNativeQuestionMarks.Value = false;
            RevealRooms.Value = true;
            RevealMode.Value = SurveyorMap.RevealMode.FullReveal;
            ShowEnemies.Value = true;
            EnemyDetectionMode.Value = SurveyorMap.EnemyDetectionMode.NearbyOnly;
            EnemyUpdateInterval.Value = 0.5f;
            EnableProfiling.Value = false;
            LogHelper.Info("Stabilized layout defaults: BottomLeft 260x260, scale=1, rotateWithPlayer=false.");

            _layoutDefaultsVersion.Value = 6;
        }
    }
    ...
    public sealed class SurveyorMapController : MonoBehaviour
    {
        ...
        private RectTransform _mapViewPivot;
        ...
        private string _lastViewTransformSummary;
        private float _nextViewTransformLog;
        private const float ViewTransformLogInterval = 10f;
        ...
        private void CreateHud()
        {
            ...
            // Rotation pivot — always stays at clipRoot center; handles RotateWithPlayer
            GameObject pivotObject = new GameObject("ViewPivot");
            pivotObject.transform.SetParent(_clipRoot, false);
            _mapViewPivot = pivotObject.AddComponent<RectTransform>();
            _mapViewPivot.anchorMin = new Vector2(0.5f, 0.5f);
            _mapViewPivot.anchorMax = new Vector2(0.5f, 0.5f);
            _mapViewPivot.pivot = new Vector2(0.5f, 0.5f);
            _mapViewPivot.anchoredPosition = Vector2.zero;
            _mapViewPivot.sizeDelta = Vector2.zero;

            // Content root — larger than the minimap window to allow panning; all map content lives here
            GameObject contentObject = new GameObject("MapContent");
            contentObject.transform.SetParent(_mapViewPivot, false);
            ...
            _contentRoot.sizeDelta = new Vector2(1024f, 1024f);
        ...
        private void UpdateSelfMarkerOverlay(Vector2 playerHudPoint)
        {
            ...
            // When CenterOnPlayer, place self marker at playerHudPoint in markerRoot space.
            // After contentRoot pans by -playerHudPoint, the marker appears at the minimap center.
            Vector2 selfPos = _config.CenterOnPlayer.Value ? playerHudPoint : Vector2.zero;
            _selfMarker.SetPosition(selfPos);
        ...
        {
            if (_mapViewPivot == null || _contentRoot == null)
                return;

            float rotAngle = _config.RotateWithPlayer.Value ? -yaw : 0f;
            _mapViewPivot.localEulerAngles = new Vector3(0f, 0f, rotAngle);
            _contentRoot.anchoredPosition = _config.CenterOnPlayer.Value ? -playerHudPoint : Vector2.zero;

            string summary = "ViewTransform: playerHudPt=(" + playerHudPoint.x.ToString("0.0") +
                             "," + playerHudPoint.y.ToString("0.0") +
                             ") pan=(" + (-playerHudPoint.x).ToString("0.0") +
                             "," + (-playerHudPoint.y).ToString("0.0") +
                             ") rotAngle=" + rotAngle.ToString("0.0") +
                             " centerOnPlayer=" + _config.CenterOnPlayer.Value +
                             " rotateWithPlayer=" + _config.RotateWithPlayer.Value;
            bool changed = _lastViewTransformSummary != summary;
            if (changed || now >= _nextViewTransformLog)
            {
                _lastViewTransformSummary = summary;
                _nextViewTransformLog = now + ViewTransformLogInterval;
                LogHelper.Debug(summary);
            }
        }
    ...
    internal sealed class NativeMapCaptureProvider
    {
        ...
        private float _renderCameraOnceAverageMs;
        ...
        public NativeMapCapture Capture(SurveyorMapConfig config, RunState runState)
        {
            ...
            RevalidateAfterCameraChange(capture);
            EnsureNativeMapReadyForCapture(map, ref capture);
            if (!capture.MapActive)
            {
                if (string.IsNullOrEmpty(capture.Reason) || capture.Reason == "not-ready")
                    capture.Reason = "native map inactive";
                HideNativeVisuals(map, _camera, ref capture);
                FinishCaptureProfile(captureStart, ref capture);
                return capture;
            }

            bool renderTextureCreated;
            RenderTexture renderTexture = ResolveRenderTexture(config, _camera, out renderTextureCreated);
            if (renderTexture == null)
            {
                capture.Reason = "camera rendertexture missing";
                HideNativeVisuals(map, _camera, ref capture);
                FinishCaptureProfile(captureStart, ref capture);
                return capture;
            }

            ConfigureTexture(renderTexture);
            float renderCameraOnceMs;
            float renderMs;
            capture.RenderResult = RenderCameraOnce(_camera, renderTexture, out renderCameraOnceMs, out renderMs);
            capture.RenderCameraOnceMs = renderCameraOnceMs;
            capture.RenderMs = renderMs;
            _renderCameraOnceAverageMs = MovingAverage(_renderCameraOnceAverageMs, renderCameraOnceMs, 0.2f);
            _renderAverageMs = MovingAverage(_renderAverageMs, renderMs, 0.2f);
            FillCameraState(_camera, ref capture);
            RevalidateAfterCameraChange(capture);
            _lastTexture = renderTexture;
            capture.Texture = renderTexture;
            capture.TextureDescription = TextureDescription(renderTexture);
            capture.Source = "surveyormap.rendertexture";
            capture.TextureHasVisiblePixels = TrySampleTexture(renderTexture, out capture.PixelSample);
            capture.Ready = capture.RenderResult == "render-ok" && capture.TextureHasVisiblePixels;
            capture.Reason = capture.Ready ? "native-map-rendered" : "rendertexture-empty";
            capture.RenderTextureCreated = renderTextureCreated;
            capture.RenderTextureCreateCount = _ownedRenderTextureCreateCount;
            capture.RenderAverageMs = _renderAverageMs;
            capture.RenderCameraOnceAverageMs = _renderCameraOnceAverageMs;
        ...
        public void ReleaseRuntimeState()
        {
            ...
            NativeMapCapture ignored = new NativeMapCapture();
            HideNativeVisuals(Map.Instance, _camera, ref ignored);
        }
        ...
        private void ReleaseOwnedRenderTexture()
        {
            ...
            // Clear targetTexture on every camera that references this RT, not just _camera.
            // Unity logs "Releasing render texture that is set as Camera.targetTexture!" when
            // any camera still holds a reference at Release() time.
            int cleared = 0;
    ...
    internal static class GameApi
    {
        ...
        public static RunState GetRunState(SurveyorMapConfig config)
        {
            ...
            try
            {
                ...
                bool gameplayState = IsGameplayState();
                ...
                if (state.PlayerApiReady && generatedLevelReady)
                {
                    state.Name = "Level";
                    state.InLevel = true;
                    state.Allowed = true;
                    state.Reason = "generated-level-ready";
                    return state;
                }
                ...
                if (config.EnableSpectatorMode.Value && state.Spectating && generatedLevelReady)
                {
                    state.Name = "Level";
                    state.InLevel = true;
                    state.Allowed = true;
                    state.Reason = "spectator-generated-level-ready";
                    return state;
                }
        ...
        private static bool SafeBool(Func<bool> method, string name)
        {
            ...
            {
                LogHelper.RateLimitedWarning("api-" + name, name + " unavailable right now: " + ex.Message, 10f);
                return false;
            }
        }
