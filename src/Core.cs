...
namespace SurveyorMap
{
    ...
    public sealed class SurveyorMapPlugin : BaseUnityPlugin
    {
        ...
        private int _toggleKeyDetectedCount;
        private string _lastToggleTimeUtc = "";
        ...
        internal string BuildTimestampUtc { get; private set; } = "?";
        internal int  PluginUpdateCount => _proofUpdateCounter;
        internal int  PluginOnGuiCount  => _pluginOnGuiCount;
        internal int  ToggleKeyDetectedCount => _toggleKeyDetectedCount;
        internal bool ControllerExists  => _controller != null;
        internal bool HudExists         => _controller != null && _controller.HudCreated;
        internal bool HudVisibleRuntime => _controller != null && _controller.HudActive;
        internal bool NativeTextureReady => _controller != null && _controller.NativeTextureReadyRuntime;
        ...
        internal string LastToggleTimeUtc   => _lastToggleTimeUtc;
        internal bool ProofHudVisible    => _proofHudVisible;

        private void OnEnable()
        {
            if (Log == null) return;
            LogHelper.Info("Plugin.OnEnable. scene=" + SceneName(gameObject) +
        ...
        {
            if (Log == null) return;
            LogHelper.Info("Plugin.OnDisable. scene=" + SceneName(gameObject) +
                           ", activeSelf=" + gameObject.activeSelf +
                           ", activeInHierarchy=" + gameObject.activeInHierarchy +
                           ", enabled=" + enabled);
        }
        ...
        {
            Instance = this;
            // ── CRITICAL: detach from BepInEx parent, set HideAndDontSave and DontDestroyOnLoad ──
            // dig-Minimap uses hideFlags=61 which forces Unity to treat the object as persistent.
            // DontDestroyOnLoad alone is insufficient in some BepInEx contexts.
            ...
            LogHelper.Info("SurveyorMap v1.0.0 loading. scene=" + SceneName(gameObject) +
            ...
            try
            {
                ...
                BuildTimestampUtc = File.GetLastWriteTimeUtc(dllPath).ToString("yyyy-MM-ddTHH:mm:ssZ");
        ...
        private void Update()
        {
            try
            {
                ...
                {
                    _toggleHandledFrame = Time.frameCount;
                    _toggleKeyDetectedCount++;
                    _lastToggleTimeUtc = System.DateTime.UtcNow.ToString("yyyy-MM-ddTHH:mm:ssZ");
        ...
        {
            LogHelper.Info("Plugin.OnDestroy called! frame=" + Time.frameCount +
                           ", t=" + Time.unscaledTime.ToString("0.0") +
                           ", updateCount=" + _proofUpdateCounter +
                           ", scene=" + SceneName(gameObject));
            if (_runtimeLoop != null)
                StopCoroutine(_runtimeLoop);
    ...
    public sealed class SurveyorMapController : MonoBehaviour
    {
        ...
        // JSON level-diagnostics state
        private string _lastRunStateForJson  = "unknown";
        private string _lastGateReasonForJson = "init";
        private string _lastCaptureReasonForJson = "not-captured";
        private bool _nativeTabOpenForJson;
        private int _tabOpenCount;
        private int _tabCloseCount;
        ...
        internal string LastRunStateForJson      => _lastRunStateForJson;
        internal string LastGateReasonForJson    => _lastGateReasonForJson;
        internal string LastCaptureReasonForJson => _lastCaptureReasonForJson;
        internal bool   NativeTextureReadyRuntime => _nativeTextureReady;
        ...
        private void RuntimeUpdateCore(string source)
        {
            ...
            _lastRunStateForJson  = runState.Name;
            _lastGateReasonForJson = runState.Reason;
        ...
        private void UpdateNativeMapTexture(RunState runState, float now)
        {
            ...
            _capturesThisWindow++;
            _lastCaptureReasonForJson = capture.Reason;
            bool tabNow = capture.RenderResult == "native-map-tab-active";
            if (tabNow && !_nativeTabOpenForJson) _tabOpenCount++;
            if (!tabNow && _nativeTabOpenForJson) _tabCloseCount++;
            _nativeTabOpenForJson = tabNow;
    ...
    internal sealed class RuntimeProbeBehaviour : MonoBehaviour
    {
        ...
        internal static void EnsureCreated()
        {
            ...
            try
            {
                ...
                string sceneName = go.scene.IsValid() ? go.scene.name : "invalid";
        ...
        private void TryWriteStateJson(string scene, float now)
        {
            try
            {
                ...
                J(sb, "buildMd5Short",     plugin != null ? plugin.BuildTagShort    : "?");
                J(sb, "assemblyLocation",  plugin != null ? plugin.AssemblyLocation : "?");
                J(sb, "timestampUtc",      plugin != null ? plugin.BuildTimestampUtc : "?");
                J(sb, "scene",             scene);
                ...
                Jb(sb, "toggleVisible",            plugin == null || plugin.ProofHudVisible);
                // Level diagnostics
                string currentRunState = plugin != null ? plugin.LastRunState : "no-plugin";
                J(sb, "currentRunState",           currentRunState);
                Jb(sb, "isLevel",                  currentRunState == "Level");
                Jb(sb, "hudVisible",               plugin != null && plugin.HudVisibleRuntime);
                Jb(sb, "nativeTextureReady",       plugin != null && plugin.NativeTextureReady);
                Jb(sb, "nativeMapCaptureReady",    plugin != null && plugin.NativeTextureReady);
                ...
                J(sb, "lastToggleTimeUtc",         plugin != null ? plugin.LastToggleTimeUtc : "");
                J(sb, "lastGateReason",            plugin != null ? plugin.LastGateReason    : "no-plugin");
                J(sb, "lastCaptureReason",         plugin != null ? plugin.LastCaptureReason : "no-plugin");
                // Errors
                Jb(sb, "screenshotTaken",          false);
                J(sb, "lastException",             _lastException);
                Jlast(sb, "lastErrorStack",        _lastErrorStack);
                sb.AppendLine("}");
        ...
        private static void Jn(StringBuilder sb, string k, double v) =>
            sb.AppendLine("  \"" + k + "\": " + v.ToString("0.###", System.Globalization.CultureInfo.InvariantCulture) + ",");
